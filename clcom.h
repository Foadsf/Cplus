...
intlist clsclintlist(intlist inlist1, int inint) {
  intlist tmplist;
  ...
  d_b = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int), NULL, &err);
  checkError(err, "Creating buffer d_b");
  ...
  err = clEnqueueWriteBuffer(commands, d_b, CL_TRUE, 0, sizeof(int), h_b, 0,
                             NULL, NULL);
  checkError(err, "Copying h_b to device at d_b");
  ...
  // Read back the results from the compute device
  err = clEnqueueReadBuffer(commands, d_c, CL_TRUE, 0, sizeof(int) * count, h_c,
                            0, NULL, NULL);
  if (err != CL_SUCCESS) {
    printf("Error: Failed to read output array!\n%s\n", err_code(err));
    exit(1);
